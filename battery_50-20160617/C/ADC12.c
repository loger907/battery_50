...
float Get_ADC(char channel)
{
  ...
  switch(channel)
  {
    ...
    case 5:
      ...
      break;
    case 6:
      ADC12MCTL0 = ADC12INCH_6;//A6
      ADC12CTL0 |= ADC12ENC + ADC12SC;             // Sampling and conversion start   
      break;
    case 7: 
      ADC12MCTL0 = ADC12INCH_7;//A7
      ADC12CTL0 |= ADC12ENC + ADC12SC;             // Sampling and conversion start   
      break;
    case 8:
     ...
     case 9:
      ...
      break;
    case 10: 
      ADC12MCTL0 = ADC12INCH_10;//A10
      ADC12CTL0 |= ADC12ENC + ADC12SC;             // Sampling and conversion start   
      break;
    case 11:
      ADC12MCTL0 = ADC12INCH_11;//A11
      ADC12CTL0 |= ADC12ENC + ADC12SC;             // Sampling and conversion start   
      break;
    case 12:
      ADC12MCTL0 = ADC12INCH_12;//A12
      ADC12CTL0 |= ADC12ENC + ADC12SC;             // Sampling and conversion start   
      break;
    case 13: 
      ADC12MCTL0 = ADC12INCH_13;//A13
      ADC12CTL0 |= ADC12ENC + ADC12SC;             // Sampling and conversion start   
      break;
    case 14:
      ADC12MCTL0 = ADC12INCH_14;//A14
      ADC12CTL0 |= ADC12ENC + ADC12SC;             // Sampling and conversion start   
      break;
  }
...
/******************************************************************************
** 函数名：Samples_Adc()
** 功能 ： 采样所有ADC通道10次，并取平均值保存到一个全局数组Adc_value[10]里面
** 参数：  采样次数
** 返回值：void
** Author:   weirongluan
*******************************************************************************/
...
{
  float ADC_temp=0;
  for(char i=0;i<10;i++)//10个通道都采样
  {
    ADC_temp = 0;//每采一个通道都要把这个变量清零
    for(char k=0;k<10;k++)//循环采样10次
    {
      ADC_temp +=Get_ADC(i);
    }
    Adc_value[count][i] = ADC_temp/10;//取平均值
  }
}
//ADC12的中断服务程序
