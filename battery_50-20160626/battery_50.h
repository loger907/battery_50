...
void Charging(void);//实现充电过程
void Step_8(void);//恒流充电的8个阶段
uint8_t IsOver_3V(void);//检查各个电池板块的压差是否超过3V，如果超过则返回-1
void PWM_Init(void);//PWM初始化
void PWM(long period,int duty);//PWM调整函数，频率+占空比
void ADC12_Init();//初始化ADC12
float Get_ADC(char channel);//获取某个通道的电压值
void Samples_Adc(uint8_t count);//第count组所有通道ADC采样10次并取平均值保存到一个数组里面
...
#define On_PWM   P2OUT |= BIT1;//使能PWM信号有效     
#define Off_PWM  P2OUT &=~BIT1;//使能PWM信号无效
